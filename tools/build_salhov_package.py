# -*- coding: utf-8 -*-
"""Build the Salhov signing package: copy PDFs + generate package.json with
signature/field spots located via pdfplumber anchors (Hebrew extracts in
...
FILES = {
    1: ("1 - הסכם מסגרת - מאיר סלהוב אינסטלציה.pdf", "1 - הסכם מסגרת"),
    2: ("2 - נספח א - תנאים מסחריים - מאיר סלהוב אינסטלציה.pdf", "2 - נספח א - תנאים מסחריים"),
...
for num, (fname, display) in FILES.items():
    ...
    if num == 4:
        # part A table (page 1) — measured coordinates
        ...
        # page 2 — note maker (address + signature)
...
package = {
    "token": TOKEN,
    "client": 'מאיר סלהוב אינסטלציה בע"מ',
    "client_hp": "514328541",
    "fields": [
