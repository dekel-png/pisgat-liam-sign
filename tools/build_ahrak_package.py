# -*- coding: utf-8 -*-
"""Build the Itay Ahrak signing package: copy PDFs + generate package.json
with signature/field spots located via pdfplumber anchors (Hebrew extracts in
visual order, so anchors are matched reversed).

Doc-4 fixed coordinates measured on the Ahrak PDFs (page 2 matches the Salhov
layout — the long client name reflows the intro paragraph identically)."""
import json
import os
import shutil
import sys
...
FILES = {
    1: ("1 - הסכם מסגרת - איתי אחרק.pdf", "1 - הסכם מסגרת", "הסכם מסגרת"),
    2: ("2 - נספח א - תנאים מסחריים - איתי אחרק.pdf", "2 - נספח א - תנאים מסחריים", "נספח א — תנאים מסחריים"),
    3: ("3 - נספח ב - טופס הזמנת עובדים - איתי אחרק.pdf", "3 - נספח ב - טופס הזמנת עובדים", "נספח ב — טופס הזמנת עובדים"),
    4: ("4 - ערבויות ובטוחות - איתי אחרק.pdf", "4 - ערבויות ובטוחות", "ערבויות ובטוחות"),
}
...
for num, (fname, display, dash_type) in FILES.items():
    ...
    if num == 4:
        ...
        spots.append({"type": "text", "page": 2, "x0": 61.0, "x1": 171.9, "bottom": 317.9,
                      "field": "company_address", "size": 9})
        spots.append({"type": "sig", "page": 2, "x0": 61.0, "x1": 253.8, "bottom": 334.8, "h": 26})
        # page 2 — aval guarantor #1 (rightmost column)
        spots.append({"type": "text", "page": 2, "x0": 437.7, "x1": 515.4, "bottom": 494.5,
                      "field": "full_name", "size": 8})
        spots.append({"type": "text", "page": 2, "x0": 434.8, "x1": 512.6, "bottom": 510.2,
                      "field": "id_number", "size": 8})
        spots.append({"type": "text", "page": 2, "x0": 403.5, "x1": 514.7, "bottom": 526.0,
...
package = {
    "token": TOKEN,
    "client": 'איתי אחרק עבודות קבלניות בע"מ',
    "client_hp": "516844628",
    "fields": [
